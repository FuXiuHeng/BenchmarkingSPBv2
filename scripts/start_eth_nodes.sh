...
do
    nohup geth --datadir eth_nodes/$node --port ${eth_port[$node]} --rpc --rpcapi="db,eth,net,web3,personal,admin,miner" --rpcaddr 127.0.0.1 --rpcport ${rpc_port[$node]} --rpccorsdomain "*" --networkid 881188 --nat extip:127.0.0.1 --nodiscover 1>/dev/null 2>/dev/null &
done
