...
def log_time_sent(simulation_name, time_sent, from_addr, ctp_hash):
    file_name = './log/spb/{}/results/time_sent.log'.format(simulation_name)
    os.makedirs(os.path.dirname(file_name), exist_ok=True)
    f = open(file_name, 'a+')
...
def log_time_mined(simulation_name, time_mined, from_addr, ctp_hash):
    file_name = './log/spb/{}/results/time_mined.log'.format(simulation_name)
    os.makedirs(os.path.dirname(file_name), exist_ok=True)
    f = open(file_name, 'a+')
...
def log_gas_used(simulation_name, gas_used, from_addr, ctp_hash):
    file_name = './log/spb/{}/results/gas_used.log'.format(simulation_name)
    os.makedirs(os.path.dirname(file_name), exist_ok=True)
    f = open(file_name, 'a+')
